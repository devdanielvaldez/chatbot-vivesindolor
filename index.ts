...
app.get('/', async (req, res) => {
  try {
    const solicitudesSnapshot = await db.db.collection('solicitudes').get();
    const solicitudes = solicitudesSnapshot.docs.map(doc => doc.data());
    res.render('main', { solicitudes });
  } catch (error) {
    res.status(500).send('Error al obtener solicitudes: ' + error);
  }
});
...
interface ClienteEstado {
    etapa: 'inicio' | 'nombre' | 'apellido' | 'tipoIdentificacion' | 'identificacion' | 'fechaNacimiento' | 'sexo' | 'completado';
    datos: {
        nombre?: string;
        apellido?: string;
